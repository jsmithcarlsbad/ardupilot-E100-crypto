...
public:
    ...
    /*
      Read and decrypt an encrypted file, returning decrypted data
      
      @param filename: Path to encrypted file (e.g., "file.enc")
      @param plaintext: Output buffer for decrypted data (caller must free)
      @param plaintext_len: Output length of decrypted data
      @return: true on success, false on failure
     */
    static bool read_and_decrypt_file(const char *filename, uint8_t **plaintext, size_t *plaintext_len);
    
    /*
      Read, decrypt, and display an encrypted file via GCS messages
      
      @param filename: Path to encrypted file (e.g., "file.enc")
      @param max_display_len: Maximum length to display (0 = no limit, but will be capped at reasonable size)
      @return: true on success, false on failure
     */
    static bool read_decrypt_and_display_file(const char *filename, size_t max_display_len = 0);
};
