--[[
    ...
    DESCRIPTION:
    This Lua script sets a new encryption key password in the LAS_CRYPT_KEY parameter
    and verifies that it was programmed successfully. The LAS_CRYPT_KEY parameter is
    ...
    CONFIGURATION:
    - NEW_PASSWORD: The INT32 value to set as the LAS_CRYPT_KEY parameter
      This value will be used to derive the 32-byte encryption key stored in secure storage.
      Range: -2147483648 to 2147483647 (INT32 limits)
    
    OUTPUT:
    The script will send messages to the GCS console (Mission Planner) with:
    - Current LAS_CRYPT_KEY value (if set)
    - Confirmation of setting attempt
    - Success message if verification passes
    - Error messages if setting or verification fails
    
    EXAMPLE OUTPUT:
    - "Current LAS_CRYPT_KEY: 74768360"
    - "Attempted to set LAS_CRYPT_KEY to: 74768361"
    - "SUCCESS: LAS_CRYPT_KEY verified as 74768361"
...
local success, err = pcall(function()
    ...
    --[[
        STEP 1: Read the current LAS_CRYPT_KEY parameter value
        This displays what the current value is before we change it
    --]]
    local old_value = param:get('LAS_CRYPT_KEY')
    
    if old_value then
        -- Display the current value to the console
        gcs:send_text(6, string.format('Current LAS_CRYPT_KEY: %d', old_value))
    else
        -- Parameter doesn't exist or isn't set yet
        gcs:send_text(6, 'LAS_CRYPT_KEY not found or not set')
    end
    ...
    if not set_success then
        ...
        if verify_value then
            ...
            if verify_value == NEW_PASSWORD then
                ...
                gcs:send_text(6, string.format('SUCCESS: LAS_CRYPT_KEY verified as %d', verify_value))
            else
            ...
            gcs:send_text(3, 'VERIFICATION FAILED: Could not read LAS_CRYPT_KEY after setting')
        end
    end
end)
