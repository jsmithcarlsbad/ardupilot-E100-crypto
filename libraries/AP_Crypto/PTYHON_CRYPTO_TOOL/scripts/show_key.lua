-- Display LAS_CRYPT_KEY and LAS_CRYPT_LVL parameter values once per second in warning color
...
local success, err = pcall(function()
    ...
    if leigh_key_param:init("LAS_CRYPT_KEY") then
        leigh_key_ok = true
    end
end)
...
success, err = pcall(function()
    ...
    if leigh_crypto_level_param:init("LAS_CRYPT_LVL") then
    ...
    local success, err = pcall(function()
        ...
        if leigh_key_ok and leigh_key_param then
            ...
            if leigh_key_value then
                ...
                table.insert(msg_parts, string.format("LAS_CRYPT_KEY: %d", key_int))
        ...
        if crypto_level_ok and leigh_crypto_level_param then
            ...
            if crypto_level_value then
                table.insert(msg_parts, string.format("LAS_CRYPT_LVL: %d", crypto_level_value))
            end
        end
