--[[
Safe Lua script to display LAS_CRYPT_KEY and key derivation inputs.
...
local function main()
    local success, err = pcall(function()
        ...
        if not p:init("LAS_CRYPT_KEY") then
            gcs:send_text(MAV_SEVERITY.ERROR, "DERIV: LAS_CRYPT_KEY not found")
            return
        end
        
        local key_val_float = p:get()
        if not key_val_float then
            gcs:send_text(MAV_SEVERITY.ERROR, "DERIV: Could not get LAS_CRYPT_KEY value")
            return
        end
