...
-- Test LAS_CRYPT_KEY access
local leigh_key = param:get('LAS_CRYPT_KEY')
if leigh_key then
    gcs:send_text(6, string.format("TEST: LAS_CRYPT_KEY = %d", leigh_key))
else
    gcs:send_text(6, "TEST: LAS_CRYPT_KEY not found or not set")
end

gcs:send_text(6, "TEST: Hello World from Encrypted LUA")
